...
```
        ...
        -(id)updateFromJSON:(NSDictionary*)data inContext:(NSManagedObjectContext*)ctx;
```

    * export - generates code for  -(NSDictionary*)jsonExport;

    * relatedByAttribute - specifies an attribute name. when importing a new json record, lookup if there is an existing object in CoreData that has the same value for this attribute as the record being imported.
